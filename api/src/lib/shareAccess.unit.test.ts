...
// reasons a ROUTE can be visible (a Share row, or the place its `placeId`
// points at being shared). Every test below pins one arm while the other is
// absent, so a change that quietly drops an arm fails here rather than in the
// field. The waypoint half of this file went with the phase 1c fold: a
// waypoint is a place now, and a PlaceLink grants no visibility at all — so
// there is one inheritance arm left in the codebase and it is the foreign key.
...
describe("404-not-403 anti-oracle", () => {
  ...
  it("requireShareOwner gives a stranger 404 but a sharee 403", () => {
    try {
      requireShareOwner("none", "route", "Only the owner can do this");
      throw new Error("expected throw");
    } catch (error) {
      expect((error as AppError).statusCode).toBe(404);
    }
    try {
      requireShareOwner("shared", "route", "Only the owner can do this");
      throw new Error("expected throw");
    } catch (error) {
  ...
  it("requireShareOwner is silent for the owner", () => {
    expect(() =>
      requireShareOwner("owner", "route", "Only the owner can do this"),
    ).not.toThrow();
  });
});
...
describe("directShareeIds", () => {
  it("returns the recipients a delete must fan tombstones out to", async () => {
    ...
    expect(await directShareeIds(tx as never, "route", "rt-1")).toEqual([
      "u1",
      "u2",
    ]);
  });
...
describe("parseSharableEntityType", () => {
  // Read off the vocabulary itself rather than a list retyped here: this
  // parser's whole job is to admit exactly SHARABLE_ENTITY_TYPES, so a kind
  // that joins or leaves that list must not need this test edited to stay
  // green — it must fail until the parser follows.
  it("accepts every sharable type and nothing else", () => {
    expect(SHARABLE_ENTITY_TYPES.length).toBeGreaterThan(1);
    for (const type of SHARABLE_ENTITY_TYPES) {
...
describe("revokeAllSharesBetween", () => {
  ...
  it("deletes every share type but tombstones only the delta-synced ones", async () => {
    const tx = makeTx([
      { id: "s1", entityType: "route", entityId: "rt-0", sharedWithId: EX },
    ...
    // All four Share rows go — a job share is revoked as hard as a route.
    expect(tx.share.deleteMany).toHaveBeenCalledWith({
      where: { id: { in: ["s1", "s2", "s3", "s4"] } },
    });
    // Jobs are not delta-synced, so they get no tombstone (they reconcile on
    // the next list fetch); routes do, one per losing recipient.
    expect(tx.syncTombstone.createMany).toHaveBeenCalledWith({
      data: [
        { userId: EX, entityType: "route", entityId: "rt-0" },
  ...
  it("purges each recipient's item_shared notification", async () => {
    const tx = makeTx([
      { id: "s1", entityType: "route", entityId: "rt-1", sharedWithId: EX },
    ]);
    await revokeAllSharesBetween(tx as never, ME, EX);

...
describe("placeImportKey", () => {
  it("produces a deterministic key from name + coords", () => {
    const key1 = placeImportKey("Claustral Creek", -33.5, 150.3);
    const key2 = placeImportKey("Claustral Creek", -33.5, 150.3);
    expect(key1).toBe(key2);
    expect(key1).toMatch(/^csv:[a-f0-9]{64}$/);
  });

  it("normalizes place names so formatting noise does not change the key", () => {
  ...
  it("is stable across trivial lat/lng precision differences", () => {
    ...
    const key1 = placeImportKey("Test Place", -33.500001, 150.300009);
    const key2 = placeImportKey("Test Place", -33.500002, 150.300009);
    // These differ in the 6th decimal — toFixed(5) should collapse them.
    expect(key1).toBe(key2);
  });

  it("differs for meaningfully different coordinates", () => {
    const key1 = placeImportKey("Test Place", -33.5, 150.3);
    const key2 = placeImportKey("Test Place", -34.5, 151.3);
    expect(key1).not.toBe(key2);
  });

  it("differs for different place names", () => {
    const key1 = placeImportKey("Claustral", -33.5, 150.3);
    const key2 = placeImportKey("Kanangra", -33.5, 150.3);
    expect(key1).not.toBe(key2);
  });

  it("strips type-words so 'Bell Creek' and 'Bell' produce the same key", () => {
    const key1 = placeImportKey("Bell Creek", -33.5, 150.3);
    const key2 = placeImportKey("Bell", -33.5, 150.3);
    expect(key1).toBe(key2);
  });
});
...
describe("tripContentHash", () => {
  ...
  it("differs for different sourcePlaceName", () => {
    const h1 = tripContentHash("Claustral", "2024-01-15", null, null);

...
export async function processGeoPdfJob(jobId: string): Promise<number> {
  ...
  if (recipient?.email && wantsEmail) {
    ...
    const logoHtml = base
      ? `<p style="margin:0 0 16px"><span style="display:inline-block;background-color:#deb188;border-radius:8px;padding:12px 16px"><img src="${base}/email-logo-icon.png" alt="" width="41" height="38" style="display:inline-block;vertical-align:middle;margin-right:10px" /><img src="${base}/email-logo.png" alt="Logjam" width="160" style="display:inline-block;vertical-align:middle" /></span></p>`
      : "";

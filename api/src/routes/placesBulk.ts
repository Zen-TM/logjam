...
// POST /places/bulk — idempotent file-import endpoint
router.post(
  "/",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    // nonexistent id — so the response can't confirm a place ID exists to a
    ...
    // already an existing place (those fold into a merge below); duplicate
    ...
    // merges into different places can't both claim the same key either.
    const mergeClaimedImportKeys = new Set<string>();

    // ---- Phase 2: Build operations ----
    type CreateOp = {
      kind: "create";
      data: Prisma.PlaceCreateManyInput;
    };
    type MergeOp = {
      kind: "merge";
      placeId: string;
      mergedData: Record<string, unknown>;
      setImportKey: string | null; // Set importKey if place didn't have one.
      // Reported back so the completion screen can say WHICH places were folded
      ...
      // existing place it merged into. Both are the caller's own places.
      sourceName: string;
      targetName: string;
    };
    ...
    for (const row of rowsAfterTargetCheck) {
      ...
      // Check if a place with this importKey already exists (idempotent).
      const existingPlace = existingByKey.get(importKey);
      if (existingPlace) {
      ...
      if (resolution.kind === "merge") {
        ...
        // Create new place with importKey AND importBatchId.
    ...
    // as a structured 409 instead of a raw 500 (IMPORT-1). No place name/coords
    ...
    try {
      ...
      if (
        ...
        throw new AppError(
          409,
          "Some of these places already exist or were just added. Reload your places and try the import again.",
        );
      }
      throw err;
    }
...
// POST /places/bulk/delete — bulk delete places by ID (owner only)
router.post(
  "/delete",
  requireAuth,
  async (req: AuthenticatedRequest, res: Response) => {

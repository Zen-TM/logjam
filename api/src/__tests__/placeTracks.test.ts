...
// 0/1-track guard, and the GET /places/tracks privacy boundary.
...
describe("place tracks (fake auth)", () => {
  ...
  it("allows at most one track per place (409 on the second)", async () => {
    const placeId = await createPlace(as(ALICE_SUB));
    try {
      const first = await uploadTrack(placeId, as(ALICE_SUB));
  ...
  it("allows multiple tracks on a trip log", async () => {
    ...
    try {
      ...
      // removed with the trip↔place m2m cutover.
      const tripRes = await request(API_URL)
        .post("/trips")
        .set(as(ALICE_SUB))
        .send({ placeIds: [placeId], date: "2026-06-01" });
  ...
  it("GET /places/tracks: owner and sharee see a shared place's track, stranger does not", async () => {
    const upload = await uploadTrack(SHARED_PLACE_ID, as(ALICE_SUB));

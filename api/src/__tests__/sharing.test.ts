...
// Each test creates its own place + share and tears them down, so the
// baseline seed shares (place 1 & 2 -> bob) are left intact.
...
describe("sharing routes (fake auth = alice)", () => {
  it("POST share → GET shares → DELETE share round-trips for a friend", async () => {
    const placeId = await createPlace("CH-003 share roundtrip");
    try {
  ...
  it("POST share is rejected with 403 when target is not a friend (carol)", async () => {
    const placeId = await createPlace("CH-003 non-friend share");
    try {
      const res = await request(API_URL)
        .post(`/places/${placeId}/share`)
        .set(AUTH)
  ...
  it("POST share rejects a duplicate with 409", async () => {
    const placeId = await createPlace("CH-003 duplicate share");
    try {
  ...
  it("POST share 400s without sharedWithUserId", async () => {
    const placeId = await createPlace("CH-003 missing target");
    try {
      const res = await request(API_URL)
        .post(`/places/${placeId}/share`)
        .set(AUTH)

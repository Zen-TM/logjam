...
- **Form controls are native elements in one field's clothes.** `TextField`, `NumberField` and `Select` share an anatomy: a sentence-case label, the control, an optional `hint` (visible and read with the control, where a tooltip is neither) and `FieldError`.
  ...
  - `SwatchPicker` is a ROW, and the swatch is the control: its label at the left, the colour's name beside it, and the colour itself as a square filling the row's full height at the right. Only that square is pressable. Ten swatches laid out flat spent a block of a 380px panel displaying nine colours nobody picked, and the card that replaced them made the whole width a target whose subject was one 24px colour (operator, 2026-09-17) — so the field states its value like every other field, and the part anyone aims at is the part that presses. No caret: a colour square that opens a palette needs no second sign that it is pressable, and the caret was the widest thing in the row. **The colour's NAME stays visible**, not folded into the accessible name — it is the only part of the control that survives being unable to tell the colours apart, which is the user it is hardest for. The palette opens in a `Popover`, three swatches to a row so ten make a block taller than it is wide rather than a strip the width of the panel; inside it is unchanged — a closed list of native radios, one tab stop, arrows move the choice, the chosen swatch wears an accent ring, and picking closes and returns focus. **A swatch is named by `nameOf`, not by its hex**: "#e6194b" is a name but not a helpful one, and ten of them in a row is what a screen-reader user had to pick from. `trackColorName` (`@logjam/shared`) is the one for the route palette, so the phone's picker and this one cannot disagree about which colour is "Teal".

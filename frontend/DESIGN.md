...
- **An armed tool is a MODE with a HUD, and the HUD gets its own slot in the chrome** (mobile §2). `MapChrome`'s `hud` sits directly under the search box, measured from `--map-inset-left` like everything else, so the route tool slides clear of the panel instead of floating at a fixed `left: 50%` of the whole window as it did. Its settings live in it, not in Layers, because they change what the NEXT click does (the snap picker). It is NOT in the `notices` stack beneath it: that stack is `aria-live="polite"`, which is right for "Showing 42 of 298 places" and wrong for a panel holding a select and four buttons — every keystroke and every placed point would be announced. Leaving the tool discards its draft, which is why both ways out confirm.
...
- **Ways lists a fourth kind the phone has no category for.** Routes, Tracks and Imports are Logjam GPS's own three (`CATEGORY_META`); "On a place" is every track file hanging off a place, which exists here because `GET /places/tracks` is the only surface showing tracks on a place a FRIEND shared. That endpoint returns a place id, a media id and a colour and nothing else, so those rows cannot be sorted into the other three — no filename, no `origin` — and are named for their place instead. The two endpoints overlap on the user's own files, so `buildWays` de-duplicates by media id and keeps the row that knows the file's name (`waysModel.ts`).

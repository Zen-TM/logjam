...
- **Form controls are native elements in one field's clothes.** `TextField`, `NumberField` and `Select` share an anatomy: a sentence-case label, the control, an optional `hint` (visible and read with the control, where a tooltip is neither) and `FieldError`.
  ...
  - `SwatchPicker` is a FIELD showing its choice — a swatch, its label and the colour's name — that opens the palette in a `Popover`. Ten swatches laid out flat spent a block of a 380px panel displaying nine colours nobody picked (operator, 2026-09-17); a field states its value like every other field, and the palette is one press away. Inside the popover it is unchanged: a closed list of native radios, one tab stop, arrows move the choice, the chosen swatch wears an accent ring, and picking closes and returns focus to the field. **A swatch is named by `nameOf`, not by its hex**: "#e6194b" is a name but not a helpful one, and ten of them in a row is what a screen-reader user had to pick from. `trackColorName` (`@logjam/shared`) is the one for the route palette, so the phone's picker and this one cannot disagree about which colour is "Teal".

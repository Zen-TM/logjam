...
- Maps → "what maps have I made, and what is still being made?" — two views, GeoPDFs and LiDAR topos. Each hero counts the finished maps; **"Being made" leads the list**, because a wait is what someone comes back to check; then the maps newest first, then (topos) their exports, then the templates. It was a stack of job ribbons, capped at three behind a "Show all", over accordions that all opened closed — so the page's own answer was a click away.
...
- **A failed job's one verb is on the row.** "Being made" rows move a bar while running; a failed one says so in a warning pill and in the worker's own words (the export worker scrubs paths first), and its only verb — Dismiss, which deletes the dead row — is a round × on the row rather than a ⋯ holding one item.
...
- **Form controls are native elements in one field's clothes.** `TextField`, `NumberField` and `Select` share an anatomy: a sentence-case label, the control, an optional `hint` (visible and read with the control, where a tooltip is neither) and `FieldError`.
  ...
  - **`RangeField` for a number judged by watching its effect** (a label size on the live map): a native `<input type="range">` with its reading beside it, and `format` as both the reading and `aria-valuetext`. A number someone types exactly is `NumberField`.
...
- Friends, Account and Settings still render their pre-redesign content inside the new shell (`LEGACY_PAGES`).
- **Maps' dialogs are still MUI**: `GeoPdfDialog`, `TopoDialog`, `TopoExportDialog`, `TopoTemplateEditDialog` and `topoSettings/*` (Phase B package 3b/3c). The page opens them unchanged. `ShareDialog` (package 4) says a topo's recipients "cannot re-export it", which is wrong — Export is offered to them, and POST /topo-exports accepts a shared source — and is to be fixed in its own commit.

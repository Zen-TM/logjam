...
describe("normalizePlaceName", () => {
  it("despaces and strips type words", () => {
    const n = normalizePlaceName("Ball Race Canyon");
  ...
  it("strips noise words", () => {
    expect(normalizePlaceName("Bell Creek Complete").base).toBe("bell");
  });

  it("extracts and relocates qualifiers, mapping abbreviations", () => {
    const a = normalizePlaceName("Upper Deanes Creek");
    const b = normalizePlaceName("Deanes Creek (Upper)");
...
describe("nameTier", () => {
  it("exact when despaced bases and qualifiers match", () => {
    expect(
      nameTier(
        normalizePlaceName("Bubble-Bath"),
        normalizePlaceName("Bubble Bath"),
      ),
    ).toBe("exact");
  });
  it("typo within edit distance with equal qualifiers", () => {
    expect(
      nameTier(
        normalizePlaceName("Sassafrass"),
        normalizePlaceName("Sassafras"),
      ),
    ).toBe("typo");
  });
  it("qualifier-mismatch when bases match but qualifiers differ", () => {
    expect(
      nameTier(
        normalizePlaceName("Bowens Creek South"),
        normalizePlaceName("Bowens Creek North"),
      ),
  ...
  it("none for unrelated names", () => {
    expect(
      nameTier(
        normalizePlaceName("Claustral"),
        normalizePlaceName("Glow Worm"),
      ),
    ).toBe("none");
  });
  ...
  // typo, silently merging two different places.
  it("none when number tokens differ", () => {
    expect(
      nameTier(
        normalizePlaceName("Wollangambe Two"),
        normalizePlaceName("Wollangambe One"),
      ),
    ).toBe("none");
  });
  it("none when a number word and a digit disagree", () => {
    expect(
      nameTier(
        normalizePlaceName("Wollangambe Two"),
        normalizePlaceName("Wollangambe 1"),
      ),
    ).toBe("none");
  });
  it("none when only one side carries a number", () => {
    expect(
      nameTier(
        normalizePlaceName("Wollangambe Complete"),
        normalizePlaceName("Wollangambe One"),
      ),
    ).toBe("none");
  });
  it("none for a bare name against a numbered one", () => {
    expect(
      nameTier(
        normalizePlaceName("Wollangambe"),
        normalizePlaceName("Wollangambe Two"),
      ),
    ).toBe("none");
  });
  ...
  it("still tolerates a genuine typo alongside matching numbers", () => {
    expect(
      nameTier(
        normalizePlaceName("Wollangambe Two"),
        normalizePlaceName("Wollangambee Two"),
      ),
    ).toBe("typo");
  });
  it("still tolerates a genuine typo when neither name has a number", () => {
    expect(
      nameTier(
        normalizePlaceName("Clasutral"),
        normalizePlaceName("Claustral Canyon"),
      ),
    ).toBe("typo");
  });
});

// The 19 positive corpus pairs (plan §5a). CSV/place imports carry coords, so
...
describe("matchPlace — negative and ambiguity cases", () => {
  ...
  it("ambiguous no-coords (three Waterfall Creeks) -> review", () => {
    const result = matchPlace({ name: "Waterfall Creek" }, [
  ...
  it("single exact name with no coords -> auto", () => {
    const result = matchPlace({ name: "Claustral" }, [
  ...
  it("typo with no coords -> review (no corroboration)", () => {
    const result = matchPlace({ name: "Sassafrass" }, [
  ...
  it("exact name with coords ~500m apart -> auto (tolerates cross-dataset coord drift)", () => {
    // The places.csv/RopeWiki scenario: identical name, coords a few hundred

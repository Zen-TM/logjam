// Single deterministic place name + location matcher. Shared by CSV place
// import, trip-place matching, and RopeWiki dedup. Replaced the two divergent
...
// Independent datasets pin different reference points for the same place (a
...
// being offered. Beyond this radius the suggested place is still listed, still
...
// distinct place into another one and the source row is gone), so a guess the
...
// Words that add no identifying information to a place name. Curated and
// extensible. "branch", "exit", "entry" describe a route variant of the same
// place; "complete"/"full" describe doing the whole thing.
...
export function normalizePlaceName(raw: string): NormalizedName {
  ...
  // against emptying the base entirely (e.g. a place literally named
...
export function nameTier(a: NormalizedName, b: NormalizedName): NameTier {
  ...
  // A number in a place name is identity, not spelling: "Wollangambe One" and
  // "Wollangambe Two" are different places, but their bases ("wollangambe 1" /
  ...
  // identify which of the numbered places is meant, so it must not merge into
...
function bestNameTier(
  ...
  for (const name of names) {
    const tier = nameTier(inputNorm, normalizePlaceName(name));
...
// Match an incoming place/trip name (optionally with coords) against a list of
// existing places. See plan §5a for the decision procedure.
...
// Should a surfaced (confidence "review") place-import row start on "merge into
// this match", or on "create as new"?
//
// Only the place-import path asks this question: there, the two answers are
// "fold this row into that place" vs "add a new place", and the first one is
// destructive. Trip-log matching has no destructive direction (linking a trip to
// a place is reversible and creates nothing), so it does not use this and keeps
...
// too. (Place-import rows always have coords, so null cannot occur on today's
...
function decideConfidence(
  ...
  if (best.tier === "exact") {
    ...
    // Exact name but coords far -> review (could be a different place of the
    // same name elsewhere).
    return "review";
  }

...
const KEEP: PlaceMergePolicy = DEFAULT_PLACE_MERGE_POLICY;
const USE_INCOMING: PlaceMergePolicy = {
  vGrade: "useIncoming",
  aGrade: "useIncoming",
...
describe("mergePlace (shared, per-field policy)", () => {
  ...
  it("keepExisting wins when both sides present", () => {
    const merged = mergePlace(existingPlace(), { vGrade: 5 }, KEEP);
    expect(merged.vGrade).toBe(3);
  });

  it("useIncoming wins when both sides present", () => {
    const merged = mergePlace(existingPlace(), { vGrade: 5 }, USE_INCOMING);
    expect(merged.vGrade).toBe(5);
  });

  it("per-field policy: one field keepExisting, another useIncoming", () => {
    const mixed: PlaceMergePolicy = {
      ...KEEP,
      hours: "useIncoming",
    };
    const merged = mergePlace(
      existingPlace({ vGrade: 3, hours: 4 }),
      { vGrade: 5, hours: 6 },
      mixed,
    );
  ...
  it("fills absent attribute keys from incoming (no data lost)", () => {
    const merged = mergePlace(
      existingPlace({ attributes: { terrain: "slot" } }),
  ...
  // must not gate a key the existing place has never seen.
  ...
  it("attributes policy is independent of the scalar field policies", () => {
    const merged = mergePlace(
      existingPlace({ vGrade: 3, attributes: { terrain: "slot" } }),

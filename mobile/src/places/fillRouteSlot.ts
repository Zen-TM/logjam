// Writing a new way into a place's one route slot — the confirm and the swap,
...
/** The user's answer to "this place already has a route". */
function confirmDisplacement(
  placeName: string,
  occupant: RouteSlotOccupant,
): Promise<boolean> {
  const confirm = routeSlotDisplaceConfirm(placeName, occupant);
 ...
 * recording, or upload a copy of a file as place media. It is called at most
 * once.
 */
export async function fillRouteSlot({
  ...
  if (!(await confirmDisplacement(placeName, occupant))) return false;
    ...
    ? () =>
        occupant.kind === "route"
          ? // Written explicitly rather than left to the server's own
            ...
            // the one place — the state the confirm just said would not exist.
            updateRouteLocal(occupant.id, { placeId: null })
          : // UNLINK, never delete: the file is the user's own import or
            // recording and lives on in Saved. Deleting it here is what the
            // copy-into-the-place model used to have to do.

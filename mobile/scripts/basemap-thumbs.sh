...
# 128px WEBP. Six requests total, not a scrape; every source here is licensed
...
#   python3 -c "from PIL import Image; Image.open('/tmp/shot.png').crop((412,900,924,1412)).resize((128,128)).save('assets/basemap-thumbs/protomaps.webp','WEBP',quality=82)"
#
...
for name in os.listdir(out):
    ...
    Image.open(path).convert("RGB").resize((128, 128), Image.LANCZOS).save(
        os.path.join(out, f"{stem}.webp"), "WEBP", quality=82, method=6
    )
    os.remove(path)
PY

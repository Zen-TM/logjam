.PHONY: dev dev-snapshot dev-cognito reset seed snapshot logs down help shared _localstack-tf geo-pdf-run
...
## Run the GeoPDF worker locally for one queued job (local dev has no ECS, so
## submitted GeoPDF jobs stay queued — this processes one). Usage:
##   make geo-pdf-run JOB=<geoPdfJobId>
## Other config (S3/AWS/LocalStack) comes from .env.local, loaded above.
geo-pdf-run:
